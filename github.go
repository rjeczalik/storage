...
func (gh *GitHubBackend) PutObject(path string, content []byte) error {
	_, path, err := gh.split(path)
	if err != nil {
		return err
	}

	if stdpath.Base(path) == "index-cache.yaml" {
		// TODO: cache in-memory instead?
		return nil // ignore to prevent polluting logs with warnings
	}

	return errors.New("not implemented: read-only storage")
}
